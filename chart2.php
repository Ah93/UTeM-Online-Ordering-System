...
$sth = oci_parse($conn,"begin getTopCustomerOrdering(:top); end;");
oci_bind_by_name($sth, ":top", $curs, -1, OCI_B_CURSOR);
oci_execute($sth);
oci_execute($curs); 
...
$table['cols'] = array(
    ...
    array('label' => 'CUSTOMER_ID', 'type' => 'string'),
...
while (($r = oci_fetch_array($curs, OCI_ASSOC+OCI_RETURN_NULLS)) != false){
    ...
    $temp[] = array('v' => (string) $r['CUSTOMER_ID']); 

    // Values of each slice
    $temp[] = array('v' => (int) $r['TOTAL_ORDERS']); 
    $rows[] = array('c' => $temp);
}
...
<link rel="stylesheet" href="css/bootstrap.min.css">
    ...
    function drawChart() {
      ...
      var options = {
           title: 'The top custumers ordering ',
          is3D: 'true',
          width: 800,
          height: 600
        };
